		...
		if (position < getItemCount()) {
			try {
				View view = recycler.getViewForPosition(0);// fix
															// ��̬���ʱ��IndexOutOfBoundsException
				if (view != null) {
					RecyclerView.LayoutParams p = (RecyclerView.LayoutParams) view
							.getLayoutParams();
					int childWidthSpec = ViewGroup.getChildMeasureSpec(
							widthSpec, getPaddingLeft() + getPaddingRight(),
							p.width);
					int childHeightSpec = ViewGroup.getChildMeasureSpec(
							heightSpec, getPaddingTop() + getPaddingBottom(),
							p.height);
					Log.i("lxy123",getTopDecorationHeight(view)+getBottomDecorationHeight(view)+"-------------");
