package com.eunut.widget.image.gif;
...
public class GifDrawable extends Drawable implements Animatable, MediaPlayerControl {
	...
	static native int openFd(int[] metaData, FileDescriptor fd, long offset, boolean justDecodeMetaData)
	...
	static native int openDirectByteBuffer(int[] metaData, ByteBuffer buffer, boolean justDecodeMetaData)
	        throws GifIOException;
	...
	public GifDrawable(String filePath) throws IOException {
		if (filePath == null) throw new NullPointerException("Source is null");
	...
	public GifDrawable(File file) throws IOException {
		if (file == null) throw new NullPointerException("Source is null");
	...
	public GifDrawable(InputStream stream) throws IOException {
		if (stream == null) throw new NullPointerException("Source is null");
		if (!stream.markSupported()) throw new IllegalArgumentException("InputStream does not support marking");
	...
	public GifDrawable(AssetFileDescriptor afd) throws IOException {
		if (afd == null) throw new NullPointerException("Source is null");
	...
	public GifDrawable(FileDescriptor fd) throws IOException {
		if (fd == null) throw new NullPointerException("Source is null");
	...
	public GifDrawable(byte[] bytes) throws IOException {
		if (bytes == null) throw new NullPointerException("Source is null");
	...
	public GifDrawable(ByteBuffer buffer) throws IOException {
		if (buffer == null) throw new NullPointerException("Source is null");
		if (!buffer.isDirect()) throw new IllegalArgumentException("ByteBuffer is not direct");
		mGifInfoPtr = openDirectByteBuffer(mMetaData, buffer, false);
		mColors = new int[mMetaData[0] * mMetaData[1]];
		mInputSourceLength = buffer.capacity();
	}
	/**
	 * Creates drawable from {@link Uri} which is resolved using
	 * {@code resolver}.
	...
	public String toString() {
		return String.format(Locale.US, "GIF: size: %dx%d, frames: %d, error: %d", mMetaData[0], mMetaData[1],
		        mMetaData[2], mMetaData[3]);
	}
	/**
	 ...
	 * 
	 * @param factor new speed factor, eg. 0.5f means half speed, 1.0f - normal,
	 *            2.0f - double speed
	 * @throws IllegalArgumentException if factor&lt;=0
	 */
	public void setSpeed(float factor) {
		if (factor <= 0f) throw new IllegalArgumentException("Speed factor is not positive");
	...
	public void seekToFrame(final int frameIndex) {
		if (frameIndex < 0) throw new IllegalArgumentException("frameIndex is not positive");
		runOnUiThread(new Runnable() {
			@Override
			public void run() {
	...
	public long getAllocationByteCount() {
		...
		if (colors == null) return nativeSize;
	 ...
	 * is a packed int representing a {@link Color}. If GifDrawable is recycled
	 * pixels[] is left unchanged.
	 * 
	 * @param pixels the array to receive the frame's colors
	 * @throws ArrayIndexOutOfBoundsException if the pixels array is too small
	 *             to receive required number of pixels
	 */
	 ...
	 * @throws IllegalArgumentException if x, y exceed the drawable's bounds or
	 *             drawable is recycled
	 */
	public int getPixel(int x, int y) {
		if (x < 0) throw new IllegalArgumentException("x must be >= 0");
		if (y < 0) throw new IllegalArgumentException("y must be >= 0");
		if (x >= mMetaData[0]) throw new IllegalArgumentException("x must be < GIF width");
		if (y >= mMetaData[1]) throw new IllegalArgumentException("y must be < GIF height");
		final int[] colors = mColors;
		if (colors == null) throw new IllegalArgumentException("GifDrawable is recycled");
	...
	public void draw(Canvas canvas) {
		...
		if (mPaint.getShader() == null) {
			if (mIsRunning) {
				if (renderFrame(mColors, mGifInfoPtr, mMetaData)) for (AnimationListener listener : mListeners)
			...
			if (mMetaData[4] >= 0 && mMetaData[2] > 1) scheduleSelf(mInvalidateTask, mMetaData[4]);//TODO don't post if progress for given frame was already posted
		} else canvas.drawRect(mDstRect, mPaint);
	}
	/**
