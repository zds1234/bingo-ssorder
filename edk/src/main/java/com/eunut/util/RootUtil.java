...
public class RootUtil {
	 ...
	 * @return
	 */
	public static boolean isRoot(Context ctx) {
		boolean bool = false;
		try {
		...
		return bool || new RootUtil().hasRootAccess(ctx);
	}
	private boolean hasRootAccess(Context ctx) {
		final StringBuilder res = new StringBuilder();
		try {
			if (runScriptAsRoot(ctx, "exit 0", res) == 0) return true;
		} catch (Exception e) {}
		return false;
	}
	private int runScriptAsRoot(Context ctx, String script, StringBuilder res) {
